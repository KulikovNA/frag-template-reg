"""DGCNN model for axisymmetric profile-coordinate prediction."""
...
class DGCNNProfile(nn.Module):
    """Predict per-point object profile coordinates ``[r, axial]``."""
    ...
    def forward(self, points: torch.Tensor | dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        pred_profile_O = self.profile_head(features).transpose(1, 2).contiguous()
        outputs = {"pred_profile_O": pred_profile_O}
        ...
        if self.axis_head is not None:
            global_features = torch.cat([features.max(dim=-1).values, features.mean(dim=-1)], dim=-1)
            outputs["pred_axis_C"] = nn.functional.normalize(self.axis_head(global_features), dim=-1)
        return outputs
